...
class ConsumerService
  ...
  def initialize
    ...
    @channel.prefetch(1)
    Rails.logger.debug 'Consumeer Channel Created'
  end
  ...
  def declare_queue(queue_name = 'default', durable = false)
    Rails.logger.debug "A queue with name #{queue_name} declared"
    @channel.queue(queue_name, durable: durable)
  end

  def consume_message(queue)
    Rails.logger.debug 'Start Consuming Message'
    begin
      ...
      queue.subscribe(manual_ack: true, block: true) do |delivery_info, _properties, payload|
        Rails.logger.debug " [x] Received '#{payload}'"
  ...
  def close_connection
    Rails.logger.debug 'Connections Closed'
